...
- **Concurrency gotcha (found and fixed 2026-07-12):** the original TTL cache was not safe under concurrent requests — a live test with 8 simultaneous `/scenario/hormuz` calls on a cold cache produced 8 real Alpha Vantage HTTP calls (and 7 cascaded EIA fallback calls) instead of 1, because multiple `asyncio` coroutines could all pass the "is the cache stale?" check before any of them finished refreshing it. A handful of concurrent demo viewers loading the page at once could have burned most of the daily quota in one burst. Fixed with an `asyncio.Lock` per cache using double-checked locking (check freshness cheaply outside the lock; if stale, acquire the lock and re-check before actually fetching, so only the first waiter performs the real HTTP call). Re-verified live post-fix: 5 concurrent requests on a cold cache → exactly 1 real Alpha Vantage call. `EiaCache` had the identical bug and got the identical fix.
...
- **Concurrency fix (2026-07-12):** `WeatherCache` had the same check-then-fetch-then-set race as `AlphaVantageCache`/`EiaCache` (see §5 above) — closed with the identical `asyncio.Lock` double-checked-locking pattern. Lower-stakes here (Open-Meteo's cap is a generous 10k req/day vs. Alpha Vantage's 25/day), but fixed for consistency. Covered by `test_cache_concurrent_requests_on_cold_cache_dedupe_to_one_fetch` in `backend/tests/test_weather.py` (10 concurrent cold-cache requests → 1 real fetch).
...
- **Concurrency fix (2026-07-12):** `FreightCache` had the same check-then-fetch-then-set race as `AlphaVantageCache`/`EiaCache` (see §5 above) — closed with the identical `asyncio.Lock` double-checked-locking pattern. Lower-stakes here (FRED has no documented hard rate cap), but fixed for consistency. Covered by `test_cache_concurrent_requests_on_cold_cache_dedupe_to_one_fetch` in `backend/tests/test_freight.py` (10 concurrent cold-cache requests → 1 real fetch).
